...
| 용어 | 종류 | 코드명 | 정의 (1–2문장) | 쓰지 말 것 | 컨텍스트 | 상태 |
|------|------|--------|---------------|-----------|---------|------|
| 주문 | 엔티티 | Order | 고객이 상품 구매를 요청한 기록. | 오더, 구매건 | 주문 | 확정 |
| 주문 취소 | 명령 | cancelOrder | … | 환불 | 주문 | 후보 |
| 고객 | 행위자 | Customer | … | 사용자, 회원 | 주문 | 충돌: 결제 컨텍스트는 "회원" 사용 |
```

- 같은 단어가 컨텍스트마다 다른 뜻이면 행을 나눈다. 억지로 하나로 합치지 않는다.
- 구현 세부(테이블명, API 경로)는 적지 않는다.
- 종류: `행위자` · `엔티티` · `값` · `상태` · `명령` · `이벤트` · `외부`. 스토리에만 나오는 행위자(예: "플랫폼")는 코드명 칸에 `(코드 개념 아님)`.
- 컨텍스트 칸은 Phase 3 전에는 `미정`. Phase 3이 끝나면 채운다.
- 상태: `후보` · `확정` · `충돌: …` · `(시뮬레이션)` 같은 출처 표시를 덧붙인다.
...
## 서브도메인
| 서브도메인 | 분류 | 근거 (경계 단서) | 컨텍스트 | 적용 수준 |
|-----------|------|-----------------|---------|----------|
| 주문 | core | 트리거 차이 (고객 요청) | ordering | DDD |
| 상품 설명 | supporting | 용어가 바뀐다 | catalog | CRUD + 검증 (가격 > 0) |
| 결제 | generic | 외부 PG가 처리 | payment | CRUD, ACL만 |
...
적용 수준이 `DDD`인 컨텍스트만 전체 캔버스를 쓴다. `CRUD`/`CRUD + 검증`은 판정·근거·검증 규칙만 몇 줄로 쓴다.
