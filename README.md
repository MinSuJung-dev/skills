...
**옵션 C — DDD 도메인 엔지니어링** (기본값: 비활성화, 복잡한 비즈니스 규칙이 있거나 `docs/domain/`이 있으면 활성화 추천)

기능 개발 파이프라인이 `ddd` 스킬 프로토콜을 따른다. `ddd` 스킬이 설치되어 있어야 한다.
- architect: 단순 CRUD인지 판정 → 아니면 설계 검증 게이트 통과 후에만 spec 작성. 스토리가 없거나 사용자 결정이 필요하면 메인에 반환
- dev: 도메인 규칙 테스트 먼저, 계층 분리, 용어집 코드명 사용
- qa: 별도 맥락에서 DDD 리뷰, HIGH 위반이나 추측된 규칙이 있으면 NEEDS_FIX
- CLAUDE.md에 도메인 섹션 추가
